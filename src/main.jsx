...
function App() {
  ...
  const submit = async () => {
    setStep('final');
    try { await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/date-confirmation`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...details, dateType: details.vibes.join(', ') }) }); setSent(true); } catch { setSent(false); }
  };
  ...
  return <main className="app-shell">
    ...
    {step === 'confirm' && <Planner eyebrow="final check · no funny business" title="Are we officially making this a date? ❤️" copy="Screenshot this. This is officially evidence. 😌❤️"><DateCard details={details} /><Next onClick={submit} text="YES, CONFIRM IT ❤️" /><button className="back-button" onClick={() => setStep('date')}>Wait, I want to change something</button></Planner>}
